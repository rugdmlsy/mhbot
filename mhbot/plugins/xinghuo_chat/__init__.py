...
def get_spark_model_info(model_version):
    model_info = {
        ...
        "Lite": {"wss": "wss://spark-api.xf-yun.com/v1.1/chat", "domain": "lite"},
    }
...
# 带上下文的聊天
chat_record = on_message(rule=to_me(), priority=10, block=True)
...
async def _(event: MessageEvent):
    ...
    if content == "" or content is None:
        # await chat_record.send(MessageSegment.text("内容不能为空！"), at_sender=True)
        return
    ...
    try:
        ...
        await chat_record.send(MessageSegment.text(res), at_sender=True)
        session[session_id].append(ChatMessage(role="assistant", content=res))
    except Exception as error:
        await chat_record.send(str(error), at_sender=True)
